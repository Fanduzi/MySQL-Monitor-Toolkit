...
echo "###############关键status检查#####################"
status(){
mysqladmin -umysql -pmysql -S /data/mysqldata/3306/mysql.sock extended-status | awk 'BEGIN { FS="|";s=0; } \
{if($2 ~ /Aborted_connects/){s=1} \
else if($2 ~ /Connections/){s=1} \
else if($2 ~ /Com_select/){s=1} \
...
help(){
while read -p "Enter line number for help: " line
do
        if [[ $line -eq 1 ]]; then
                ...
                echo "MySQL启动后执行的SELECT语句的次数"
        elif [[ $line -eq 3 ]]; then
                  echo "Connections:"
                echo "连接尝试次数（成功与否）到MySQL服务器"
        elif [[ $line -eq 4 ]]; then
                  echo "Created_tmp_disk_tables:"
